 ...
 *
 * $Id: s3c-udc-otg-hs.c,v 1.4 2007/08/13 09:25:24 dasan Exp $*
 *
...
#include <mach/regs-usb-otg-hs.h>
#include "clock.h"
                                   ...
                                   struct s3c_request *req, int max)
{
        ...
        if(ep_index(ep) == 0) {
                usb_write((1<<19)|(length<<0), (u32) S3C_UDC_OTG_DIEPTSIZ0, 0);

                in_ctrl =  __raw_readl(S3C_UDC_OTG_DIEPCTL0);
                usb_write(DEPCTL_EPENA|DEPCTL_CNAK|(BULK_IN_EP<<11)| in_ctrl, (u32) S3C_UDC_OTG_DIEPCTL0, 0);
                
                DEBUG_EP0("%s:(DIEPTSIZ0):0x%x, (DIEPCTL0):0x%x, (GNPTXSTS):0x%x\n", __FUNCTION__,
                        __raw_readl(S3C_UDC_OTG_DIEPTSIZ0),__raw_readl(S3C_UDC_OTG_DIEPCTL0),
                        __raw_readl(S3C_UDC_OTG_GNPTXSTS));
                udelay(20);
        }
        else {
                usb_write((1<<29)|(1<<19)|(length<<0), (u32) S3C_UDC_OTG_DIEPTSIZ2, 2);
                
                in_ctrl =  __raw_readl(S3C_UDC_OTG_DIEPCTL2);
                usb_write(DEPCTL_EPENA|DEPCTL_CNAK|(BULK_IN_EP<<11)| in_ctrl, (u32) S3C_UDC_OTG_DIEPCTL2, 2);

                DEBUG_EP2("%s:(DIEPTSIZ2):0x%x, (DIEPCTL2):0x%x, (GNPTXSTS):0x%x\n", __FUNCTION__,
                        __raw_readl(S3C_UDC_OTG_DIEPTSIZ2),__raw_readl(S3C_UDC_OTG_DIEPCTL2),
                        __raw_readl(S3C_UDC_OTG_GNPTXSTS));
...
/*        uTemp = __raw_readl(S3C_OTHERS);
        uTemp |= (1<<16);   // USB_SIG_MASK
        __raw_writel(uTemp, S3C_OTHERS);*/
        ...
        DEBUG_SETUP("S3C6400 USB 2.0 OTG Controller Core Initialized : 0x%x\n",  __raw_readl(S3C_UDC_OTG_GINTMSK));
...
static int write_fifo(struct s3c_ep *ep, struct s3c_request *req)
{
        ...
        if (is_last) {
                ...
                __raw_writel(gintmsk&(~INT_TX_FIFO_EMPTY), S3C_UDC_OTG_GINTMSK);
                done(ep, req, 0);
                return 1;
        }
...
static int read_fifo(struct s3c_ep *ep, struct s3c_request *req)
{
        ...
        csr = __raw_readl(S3C_UDC_OTG_GRXSTSP);
        ...
        if(!count_bytes) {
                ...
                __raw_writel(gintmsk | INT_RX_FIFO_NOT_EMPTY, S3C_UDC_OTG_GINTMSK);
                return 0;
        }
...
static void reconfig_usbd(void)
{
        ...
        u32 uTemp = __raw_writel(CORE_SOFT_RESET, S3C_UDC_OTG_GRSTCTL);
        ...
        __raw_writel(INT_RESUME|INT_ENUMDONE|INT_RESET|INT_SUSPEND|INT_RX_FIFO_NOT_EMPTY
                                ,S3C_UDC_OTG_GINTMSK);  
        
        // 7. Set NAK bit of EP0, EP1, EP2
        //u32 uTemp = __raw_readl(S3C_UDC_OTG_DOEPCTL0);
        __raw_writel(DEPCTL_EPDIS|DEPCTL_SNAK|(0<<0), S3C_UDC_OTG_DOEPCTL0); /* EP0: Control OUT */
        __raw_writel(DEPCTL_EPDIS|DEPCTL_SNAK|(0<<0), S3C_UDC_OTG_DIEPCTL0); /* EP0: Control IN */

        //uTemp = __raw_readl(S3C_UDC_OTG_DOEPCTL1);
        __raw_writel(DEPCTL_EPDIS|DEPCTL_SNAK|DEPCTL_BULK_TYPE|(0<<0), S3C_UDC_OTG_DOEPCTL1); /* EP1:Data OUT */
        __raw_writel(DEPCTL_EPDIS|DEPCTL_SNAK|DEPCTL_BULK_TYPE|(0<<0), S3C_UDC_OTG_DIEPCTL2); /* EP1:Data IN */

        // 8. Unmask EP interrupts on IN EPs : 0, 2
        //                            OUT EPs : 0, 1
        __raw_writel(((1<<BULK_OUT_EP)|(1<<CONTROL_EP))<<16|((1<<BULK_IN_EP)|(1<<CONTROL_EP)), S3C_UDC_OTG_DAINTMSK);

        // 9. Unmask device IN EP common interrupts
        __raw_writel(AHB_ERROR, S3C_UDC_OTG_DOEPMSK);

        // 10. Unmask device OUT EP common interrupts
        __raw_writel(NON_ISO_IN_EP_TIMEOUT|AHB_ERROR, S3C_UDC_OTG_DIEPMSK);

        // 11. Set Rx FIFO Size
        __raw_writel(RX_FIFO_SIZE, S3C_UDC_OTG_GRXFSIZ);        
        
        // 12. Set Non Periodic Tx FIFO Size
        __raw_writel(NPTX_FIFO_SIZE<<16| NPTX_FIFO_START_ADDR<<0, S3C_UDC_OTG_GNPTXFSIZ);               

        // 13. Clear NAK bit of EP0, EP1, EP2
        // For Slave mode
        __raw_writel(DEPCTL_EPDIS|DEPCTL_CNAK|(0<<0), S3C_UDC_OTG_DOEPCTL0); /* EP0: Control OUT */

        // 14. Initialize OTG Link Core.
        __raw_writel(PTXFE_HALF|NPTXFE_HALF|MODE_SLAVE|BURST_INCR16|GBL_INT_UNMASK, S3C_UDC_OTG_GAHBCFG);
...
void set_max_pktsize(struct s3c_udc *dev, enum usb_device_speed speed)
{
        ...
        if (speed == USB_SPEED_HIGH) {
                ...
                ep_ctrl = __raw_readl(S3C_UDC_OTG_DIEPCTL0);
                usb_write(ep_ctrl|(0<<0), (u32) S3C_UDC_OTG_DIEPCTL0, 0);

                // EP0 - Control OUT (64 bytes)
                ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL0);
                usb_write(ep_ctrl|(0<<0), (u32) S3C_UDC_OTG_DOEPCTL0, 0);
        } else {
                // EP0 - Control IN (8 bytes)
                ep_ctrl = __raw_readl(S3C_UDC_OTG_DIEPCTL0);
                usb_write(ep_ctrl|(3<<0), (u32) S3C_UDC_OTG_DIEPCTL0, 0);

                // EP0 - Control OUT (8 bytes)
                ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL0);
                usb_write(ep_ctrl|(3<<0), (u32) S3C_UDC_OTG_DOEPCTL0, 0);       
        }
                

        // EP1 - Bulk Data OUT (512 bytes)
        ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL1);
        usb_write(ep_ctrl|(ep_fifo_size<<0), (u32) S3C_UDC_OTG_DOEPCTL1, 1);

        // EP2 - Bulk Data IN (512 bytes)
        ep_ctrl = __raw_readl(S3C_UDC_OTG_DIEPCTL2);
        usb_write(ep_ctrl|(ep_fifo_size<<0), (u32) S3C_UDC_OTG_DIEPCTL2, 2);
...
{
        struct s3c_udc *dev = _dev;
        u32 intr_status, grx_status, fifoCntByte = 0;
        ...
        if (intr_status & INT_RX_FIFO_NOT_EMPTY) {
                ...
                __raw_writel(gintmsk, S3C_UDC_OTG_GINTMSK);

                grx_status = __raw_readl(S3C_UDC_OTG_GRXSTSR);
                ...
                if(fifoCntByte) {
                        ...
                        if ((grx_status & 0x1E0000) == SETUP_PKT_RECEIVED)  {
                        ...
                        } else if ((grx_status & 0x1E0000) == OUT_PKT_RECEIVED) {
                        
                                if(grx_status & BULK_OUT_EP) {
                                        ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL1);
                                        DEBUG_ISR("      => A Bulk OUT data packet received : %d bytes, (DOEPCTL1):0x%x\n", 
                                                fifoCntByte, ep_ctrl);
                                        s3c_out_epn(dev, 1);
                                        gintmsk = __raw_readl(S3C_UDC_OTG_GINTMSK);
                                        __raw_writel(ep_ctrl | DEPCTL_CNAK, S3C_UDC_OTG_DOEPCTL1);
                                }
                                else if (grx_status & CONTROL_EP) {
                                        DEBUG_EP0("      => A CONTROL OUT data packet received : %d bytes, (DOEPCTL1):0x%x\n", 
                                                fifoCntByte, ep_ctrl);  
                                }
                                else {
                                ...
                                grx_status = __raw_readl(S3C_UDC_OTG_GRXSTSP);
                        ...
                        if ((grx_status & 0x1E0000) == SETUP_TRANSACTION_COMPLETED) {   
                                ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL0);
                                DEBUG_EP0("      => A SETUP transaction completed (DOEPCTL0):0x%x\n", ep_ctrl);
                                __raw_writel(ep_ctrl | DEPCTL_CNAK, S3C_UDC_OTG_DOEPCTL0);
                                
                        } 
                        else if ((grx_status & 0x1E0000) == OUT_TRANSFER_COMPLELTED) {
                                ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL1);
        ...
        if (intr_status & INT_TX_FIFO_EMPTY) {
                DEBUG_ISR("    INT_TX_FIFO_EMPTY(GNPTXSTS):0x%x, (DIEPCTL2):0x%x, GINTMSK:0x%x\n", 
                                __raw_readl(S3C_UDC_OTG_GNPTXSTS), __raw_readl(S3C_UDC_OTG_DIEPCTL2),
                                __raw_readl(S3C_UDC_OTG_GINTMSK));
                s3c_in_epn(dev, 2);
        }
                         ...
                         gfp_t gfp_flags)
{
        ...
        if (list_empty(&ep->queue) && likely(!ep->stopped)) {
                ...
                if (unlikely(ep_index(ep) == 0)) {
                        ...
                        csr = usb_read((u32) S3C_UDC_OTG_GINTSTS, ep_index(ep));
                        ...
                        if((csr & INT_TX_FIFO_EMPTY)
                           && (write_fifo(ep, req) == 1))
                                ...
                                DEBUG("++++ IN-list_add_taill::req=%p, DIEPCTL2:0x%x\n", req,
                                        __raw_readl(S3C_UDC_OTG_DIEPCTL2));
                        ...
                        if((csr & INT_RX_FIFO_NOT_EMPTY)
                           && (read_fifo(ep, req) == 1))
                                ...
                                DEBUG("++++ OUT-list_add_taill::req=%p, DOEPCTL1:0x%x\n", req,
                                        __raw_readl(S3C_UDC_OTG_DOEPCTL1));
                }
        }
...
static __inline__ int s3c_fifo_read(struct s3c_ep *ep, u32 *cp, int max)
{
        int bytes;
        int count;
        u32 grx_status = __raw_readl(S3C_UDC_OTG_GRXSTSP);
        ...
        while (count--) {
                *cp++ = (u32) __raw_readl(S3C_UDC_OTG_EP0_FIFO);
        }
...
static void udc_set_address(struct s3c_udc *dev, unsigned char address)
{       
        u32 tmp = __raw_readl(S3C_UDC_OTG_DCFG);
        __raw_writel(address << 4 | tmp, S3C_UDC_OTG_DCFG);

        tmp = __raw_readl(S3C_UDC_OTG_DIEPCTL0);
        __raw_writel((1<<31)|DEPCTL_CNAK|tmp, S3C_UDC_OTG_DIEPCTL0); /* EP0: Control IN */
        
        DEBUG_EP0("%s: USB OTG 2.0 Device address=%d, DCFG=0x%x\n",
                __FUNCTION__, address, __raw_readl(S3C_UDC_OTG_DCFG));
...
static void s3c_ep0_setup(struct s3c_udc *dev)
{
        ...
        switch (ctrl.bRequest) {
                ...
                case USB_REQ_SET_CONFIGURATION :
                        ...
                        ep_ctrl = __raw_readl(S3C_UDC_OTG_DIEPCTL0);
                        __raw_writel(DEPCTL_EPENA|DEPCTL_CNAK|ep_ctrl, S3C_UDC_OTG_DIEPCTL0); /* EP0: Control IN */

                        // For Startng EP1 on this new configuration
                        ep_ctrl = __raw_readl(S3C_UDC_OTG_DOEPCTL1);
                        __raw_writel(DEPCTL_EPDIS|DEPCTL_CNAK|DEPCTL_BULK_TYPE|DEPCTL_USBACTEP|ep_ctrl, S3C_UDC_OTG_DOEPCTL1); /* EP1: Bulk OUT */

                        // For starting EP2 on this new configuration
                        ep_ctrl = __raw_readl(S3C_UDC_OTG_DIEPCTL2);
                        __raw_writel(DEPCTL_BULK_TYPE|DEPCTL_USBACTEP|ep_ctrl, S3C_UDC_OTG_DIEPCTL2); /* EP2: Bulk IN */

                        DEBUG_SETUP("%s:(DOEPCTL1):0x%x, (DIEPCTL2):0x%x\n", __FUNCTION__, 
                                __raw_readl(S3C_UDC_OTG_DOEPCTL1), __raw_readl(S3C_UDC_OTG_DIEPCTL2));
                        DEBUG_SETUP("============================================\n");
...
static int s3c_udc_get_frame(struct usb_gadget *_gadget)
{
        ...
        u32 frame = __raw_readl(S3C_UDC_OTG_DSTS);
